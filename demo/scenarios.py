...
SCENARIOS = [
    ...
    DemoScenario(
        ...
        input_text="[占位]广告引流内容示例[/占位]",
        expected_action="desensitize",
        display_purpose="证明系统支持脱敏后继续服务",
    ),
    DemoScenario(
        name="输出复检",
        description="模拟大模型输出违规内容，复检拦截",
        input_text="[占位]正常输入但模型输出违规[/占位]",
        expected_action="output_recheck",
        display_purpose="证明系统具备输出侧兜底能力",
    ),
    DemoScenario(
        name="混淆变体",
        description="使用空格、符号等规避手段的违规内容",
        input_text="[占位]混淆变体违规内容示例[/占位]",
        expected_action="block",
